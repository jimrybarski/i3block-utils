...
fn result_main() -> Result<String, &'static str> {
    ...
    let battery_information = str::from_utf8(&output.stdout).unwrap().trim_right();
    ...
    let battery_state = match caps.get(1) {
        Some(m) => { match m.as_str() {
            ...
            _ => { return Result::Err("Invalid battery state"); }
        }},
        None => { return Result::Err("Invalid battery state"); }
    };

    let power_level = match caps.get(2) {
        Some(p) => match p.as_str().parse::<u8>() {
            Ok(level) => level,
            Err(_) => { return Err("Invalid power level"); }
        },
        None => { return Result::Err("Invalid power level"); }
    };
